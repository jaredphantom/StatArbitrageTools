...
def sendBacktestResults(coins: Dict[str, np.ndarray], constant: float = 1.5, graph: bool = False, 
    ...
    with redirect_stdout(f):
        backtest.backtest(coins=coins, constant=constant, graph=graph, start=start, end=end)
...
def generateSignals(start: dt = dt(2021, 1, 1), end: dt = None):
    if end is None:
        end = dt.today()

    corr = portfolio.generateCorrMatrix(coins=statarb.dydxCrypto, threshold=0.8, start=start, end=end)
    ...
    for pair in goodPairs:
        ...
        if spreadGraph[-1] < mean - (1.5 * stddev):
            ...
            sendBacktestResults(cryptoDict, start=start, end=end)
...
if __name__ == "__main__":
    try:
        generateSignals(start=dt(2021, 1, 1))
    except ValueError:
        generateSignals(start=dt(2021, 1, 1), end=(dt.today() - td(days=1)))
